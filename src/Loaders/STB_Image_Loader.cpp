...
Image* STB_Image_load_rgba(const uint8_t* ptr, int width, int height) {
...
Image* STB_Image_load_path(const std::filesystem::path& path) {
	int nrChannels;
...
Image* STB_Image_load_bytes(const std::shared_ptr<char>& bytes, size_t bytes_length) {
	int nrChannels;
    int width = 0, height = 0;
...
Image* dz::loaders::STB_Image_Loader::Load(const dz::loaders::STB_Image_Info& info) {
    if (!info.path.empty())
